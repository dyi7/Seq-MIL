...
def bilstm_filter(input, mask, keep_prob, prefix='lstm', dim = 50, is_training=True):
   ...
   # output shape: batch*news,sequence,2*lstm_units  (32*40)*12*600(nin)  -->元组，不是单个连接的tensor,
   #,_:output_states：[last_state_fw,last_state_bw],也是元组   shape:sequence,2*lstm_units
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('sentence_enc'):
            ...
            with tf.variable_scope(name_or_scope='sentence_enc', reuse=tf.AUTO_REUSE):
                ...
                word_encoder_out = bilstm_filter(word_level_inputs, word_level_mask, keep_prob,prefix='sequence_encode', dim=options['dim'],is_training=is_training)  
                ...
                ## att shape 32*40*100
                ## TODO add context vector
                #--------------------------------------
                context_ = bilstm_filter(att, x_mask, keep_prob, prefix='sequence_context', dim=options['dim2'],is_training=is_training)  
                # output shape: batch,news,2*dim2   32*40*600
                context_out = tf.reduce_mean(tf.concat(context_, 2),1) #32,2*dim2
                #-------------------------------------要不要平均？还是直接保留N？context_out = tf.concat(context_, 2) #32,N,2*dim2
                ##TODO concat  word level + instance level bi-LSTM
                context_out_expand = tf.expand_dims(context_out,1) #32,1,2*dim2
                ouput = tf.concat([att,tf.tile(context_out_expand,[1,N,1])],2) #32,N,(2*dim  +  2*dim2)
                #-------------------------ouput = tf.concat([att,context_out],2) #32,N,(2*dim  +  2*dim2)
                #if options['use_dropout']:
                #    ouput = layers.dropout(ouput, keep_prob=keep_prob, is_training=is_training,seed=None)    
        ...
        with tf.name_scope('instance_prediction'):
            D = ouput.get_shape().as_list()[-1]
            _input = tf.reshape(ouput,[batch*N, -1])  #32*N,600
            ...
            inst_pred = tf.sigmoid(tf.matmul(_input,theta))  #32*N,1
            L = tf.reshape(inst_pred,[batch,N])  #32,N
            #print(inst_pred)"""
            ...
            x_simil = Euclidean_distance(ouput)  #32,N,N   有placeholder
            ...
            #group_cost = tf.cast(-y*tf.log(group_pred) - (1-y)*tf.log(1-group_pred),tf.float32)     ## int64
...
def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, group_pred, true_pos,true_neg,false_pos,false_neg, summary, eidx,
                    is_training,train_op, plot=None,writer=None,validate=False):
    #pred-->group_pred
    ...
    for x_sent, y_sent in iterator:
        ...
        loss, result, preds,TP,TN,FP,FN = sess.run([cost, correct_pred, group_pred,true_pos,true_neg,false_pos,false_neg],  #pred(32,2)-->group_pred(32,)
        ...
        precision = TP / (TP + FP + 1e-8) #Tensor("truediv_224:0", shape=(), dtype=float32)???
    ...
    final_mcc = (TP * TN - FP * FN) / numpy.sqrt(mm)
    ...
    return final_acc, final_loss,final_mcc, final_result
...
def train(
    ...
    with tf.variable_scope('train'):
        ...
        true_pos = tf.cast(tf.count_nonzero(group_pred * y), tf.float32)  #int64
        true_neg = tf.cast(tf.count_nonzero((group_pred - 1) * (y - 1)), tf.float32)   #int64
        false_pos = tf.cast(tf.count_nonzero(group_pred * (y - 1)), tf.float32)  #int64
        false_neg = tf.cast(tf.count_nonzero((group_pred - 1) * y), tf.float32)    #int64
        ...
        for eidx in range(max_epochs):
            ...
            for x,y in train:
                ...
                if x is None:
                    ...
                    continue
                ud_start = time.time()
                _, loss,loss_no_mean,temp_acc,TP,TN,FP,FN,l2_check = sess.run([train_op, op_loss,cost,temp_accuracy,true_pos,true_neg,false_pos,false_neg,op_L2], 
                ...
                precision = TP / (TP + FP + 1e-8) #epsilon的意义在于防止分母为0，否则当分母为0时python会报错)
                recall = TP / (TP + FN + 1e-8)
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    #pred-->group_pred
                    valid_acc, valid_loss, valid_mcc, valid_final_result = predict_pro_acc(sess, cost, prepare_data, model_options, valid, maxlen,correct_pred, group_pred, true_pos,true_neg,false_pos,false_neg, summary, eidx, is_training, train_op,loss_plot,validate_writer,validate=True)
                    test_acc, test_loss,test_mcc,test_final_result = predict_pro_acc(sess, cost, prepare_data, model_options, test, maxlen,correct_pred, group_pred, true_pos,true_neg,false_pos,false_neg, summary, eidx, is_training, train_op,loss_plot, testing_writer)
                    #valid_acc, valid_loss,valid_final_result = predict_pro_acc(sess, cost, prepare_data, model_options, valid, maxlen,
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        #pred-->group_pred
        valid_acc, valid_cost,valid_mcc,valid_final_result = predict_pro_acc(sess, cost, prepare_data, model_options, valid, maxlen, correct_pred, group_pred, true_pos,true_neg,false_pos,false_neg,summary, eidx,train_op, is_training, None)
        ...
        test_acc, train_cost, test_mcc, test_final_result =  predict_pro_acc(sess, cost, prepare_data, model_options, test,maxlen, correct_pred, group_pred, true_pos,true_neg,false_pos,false_neg,summary, eidx,train_op, is_training, None)
        ...
        train_acc, train_cost, train_mcc, _ =  predict_pro_acc(sess, cost, prepare_data, model_options, train_valid,maxlen, correct_pred, group_pred, true_pos,true_neg,false_pos,false_neg, summary, eidx,train_op, is_training, None)
