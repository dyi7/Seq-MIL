...
#改动6： 去掉alpha_balance   --> 效果越来越差0.5340909090909091
#改动7： 加入tech -->0.57  Not good(改data_iterator,改main_new...)
#改动8： word embedding(avg)-->MIL representation --> add time effects --> day attention (改data_iterator,改main_new...)


#超参：attention_v2(nin=600-->100);lstm_filter/bilstm_filter(dim=300-->50)
...
from data_iteratorMIL_day import TextIterator
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('Group_prediction'):  
            ...
            tech_ind = lstm_filter(tech, tf.ones(shape=[tf.shape(tech)[0],tf.shape(tech)[1]]), keep_prob, prefix='tech_lstm', dim=50, is_training=is_training) #32,N,50
...
def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, group_pred, summary, eidx,
    ...
    for x_sent, y_sent,y_tech in iterator:
        num += len(x_sent)
        ...
        for eidx in range(max_epochs):
            ...
            for x,y,y_tech in train:
                ...
                _, loss,loss_no_mean,temp_acc,l2_check = sess.run([train_op, op_loss,cost,temp_accuracy,op_L2],
                                   feed_dict={'input/x:0': data_x, 'input/x_mask:0': data_x_mask, 'input/y:0': data_y,'input/technical:0':y_tech,
