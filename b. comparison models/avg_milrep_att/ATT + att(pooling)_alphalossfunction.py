...
######### ATT + MLP
#改动1：group avg变为group attention  -->not working
#改动2：去掉alpha_balance,correct_pred,使用attention-mil, bag
#改动3：用自己的300 vector,改参数  -->没弄
#改动4：加上x_mask, inst_pred = tf.nn.tanh(pred_sig_) softmax改为tanh
#改动5： loss function 改为add instance function, inst_pred = tf.nn.sigmoid(pred_sig_) tanh改为sigmoid; L = tf.reshape(inst_pred,[batch,N])   l = tf.reshape(inst_pred,[batch,N,1])
#改动5：group_pred, correct_pred,group_cost  +alpha_balance  0.4602272727272727-->差
#改动6： 去掉alpha_balance
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('instance_prediction'):
            ...
            inst_pred = tf.nn.sigmoid(pred_sig_)#32*N,1 NOT 32,N,1, float32
            ##tf.sigmoid 改成 tf.relu
            inst_temp = tf.nn.tanh(pred_sig_)
            L = tf.reshape(inst_pred,[batch,N])
            l = tf.reshape(inst_temp,[batch,N,1])
        ...
        ##TODO make group prediction through attention pooling representation
        with tf.name_scope('Group_prediction'):  
            ...
            group_ = tf.layers.dense(logit, 1, activation=None, use_bias=True,
                                     kernel_initializer=layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),
                                     name='fout', reuse=tf.AUTO_REUSE)#32,1
            group_pred = tf.squeeze(tf.nn.sigmoid(group_))  #32,1 --> squeeze  32,
            group_final = tf.cast(tf.ceil(group_pred-0.5),tf.int64)    #32,
            ################################group_pred, correct_pred,group_cost     
            ##TODO new cost
            logger.info('Building f_cost...')
            x_simil = Euclidean_distance(att)  #32,N,D-->32,N,N   有placeholder
            l_diff = instance_diff(L) #32,N-->32,N,N   有placeholder
            simil_cost = tf.reduce_sum(tf.multiply(x_simil,l_diff),[1,2])/tf.cast(N*N,tf.float32)  #32,
            group_cost = tf.cast(tf.square(y-group_final),tf.float32)  #32
            # cost由int64变为float32
            total_cost = simil_cost +  group_cost  #alpha_balance *[32,1]
            cost = tf.reshape(total_cost,(1,-1))  #1,32
            """group_ = tf.layers.dense(logit, 2, activation=None, use_bias=True,
                                     kernel_initializer=layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),
            ...
            cost = tf.nn.softmax_cross_entropy_with_logits_v2(logits=group_, labels=labels)  #1,32"""
...
def train(
    ...
    with tf.variable_scope('train'):
        ...
        correct_pred = tf.equal(tf.cast(tf.round(group_pred),tf.int64), y)  # make prediction   
