...
def sentence_embedding(emb, sequence_mask, instance_mask, keep_prob, is_training, options):
    ...
    # word_level_output shape is (32*40)*600(dim*2)
    ################################### TODO average word
    ...
    instance_level_input = tf.reshape(word_level_output, [batch, instance_len, 2*options['dim']])  # 32*40*600(2*options['dim'])
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('instanceprediction'):
            ...
            #theta = tf.Variable(tf.random_normal_initializer(shape=[emb_size, 1]),stddev=0.1)
...
def train(
        ...
        dim = 50, #不变，为了保持最后output=2*50=100,  # the number of GRU units
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/Saved loss_plot pickle')
            with open("results/important_plot.pickle",'wb') as handle:
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        if os.path.exists('results/history_predict.npz'):
            logger.info("Load and save to history_predict.npz")
            valid_history = numpy.load('results/history_predict.npz')['valid_final_result']
            test_history = numpy.load('results/history_predict.npz')['test_final_result']
            ...
            numpy.savez('results/history_predict.npz',valid_final_result=vv,test_final_result=tt,**params)
        else:
            numpy.savez('results/history_predict.npz',valid_final_result=valid_final_result,test_final_result=test_final_result,**params)
