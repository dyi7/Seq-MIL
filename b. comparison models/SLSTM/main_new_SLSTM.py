# -*- coding: utf-8 -*-
"""
Created on Wed Jun 10 13:46:23 2020
...
#4. Train:100-300
#5. jj--kk
#Change word-emb=300; dim=100/300
...
from data_iterator_SLSTM import TextIterator
...
def prepare_data(sequence, sequence_d1, sequence_d2, labels, options, maxlen=None, max_word=100):
    ...
    maxlen_x = numpy.max(length)  # max time step( max news number)
    try:
    ...
    try:
        max_sequence = max(len(j) for i in sequence for z in i for j in z)  # find the sequence max length
        max_sequence_d1 = max(len(k) for i in sequence_d1 for z in i for j in z for k in j)
        max_sequence_d2 = max(len(k) for i in sequence_d2 for z in i for j in z for k in j)
        max_sentence = max(len(j) for i in sequence for j in i)
        max_sentence_d1 = max(len(j) for i in sequence_d1 for z in i for j in z)
        max_sentence_d2 = max(len(j) for i in sequence_d2 for z in i for j in z)
    except ValueError as e:
    ...
    ##TODO for x
    x = numpy.zeros((n_samples, maxlen_x, max_sentence, max_sequence)).astype('int64')
    x_mask_sentence = numpy.zeros((n_samples, maxlen_x, max_sentence)).astype('float32')
    x_mask_news = numpy.zeros((n_samples, maxlen_x)).astype('float32')
    ##TODO for x_d1
    x_d1 = numpy.zeros((n_samples, day1, maxlen_xd1, max_sentence_d1, max_sequence_d1)).astype('int64')
    x_d1_mask_sentence = numpy.zeros((n_samples, day1, maxlen_xd1, max_sentence_d1)).astype('float32')
    x_d1_mask_news = numpy.zeros((n_samples, day1, maxlen_xd1,)).astype('float32')
    ##TODO for x_d2
    x_d2 = numpy.zeros((n_samples, day2, maxlen_xd2, max_sentence_d2, max_sequence_d2)).astype('int64')
    x_d2_mask_sentence = numpy.zeros((n_samples, day2, maxlen_xd2, max_sentence_d2)).astype('float32')
    x_d2_mask_news = numpy.zeros((n_samples, day2, maxlen_xd2)).astype('float32')
    ...
    for index, (i, j, k, ll) in enumerate(zip(sequence, sequence_d1, sequence_d2, labels)):  # batch size
        ...
        for idx, ss in enumerate(i):  # time step
            for m, n in enumerate(ss):
                if len(n) < max_sequence:
                    x[index, idx, m, :len(n)] = n
                else:
                    x[index, idx, m, :max_sequence] = n[:max_sequence]
                x_mask_sentence[index, idx, m] = 1.
            x_mask_news[index, idx] = 1.
        ##TODO x_d1
        for jj, day in enumerate(j):
            for idx, ss in enumerate(day):
                for m, n in enumerate(ss):
                    if len(n) < max_sequence_d1:
                        x_d1[index, jj, idx, m, :len(n)] = n
                    else:
                        x_d1[index, jj, idx, m, :max_sequence_d1] = n[:max_sequence_d1]
                    x_d1_mask_sentence[index, jj, idx, m] = 1.
                x_d1_mask_news[index, jj, idx] = 1.
        ##TODO x_d2
        for kk, day in enumerate(k):
            for idx, ss in enumerate(day):
                for m, n in enumerate(ss):
                    if len(n) < max_sequence_d2:
                        x_d2[index, kk, idx, m, :len(n)] = n
                    else:
                        x_d2[index, kk, idx, m, :max_sequence_d2] = n[:max_sequence_d2]
                    x_d2_mask_sentence[index, kk, idx, m] = 1.
                x_d2_mask_news[index, kk, idx] = 1.

    haha = numpy.absolute(numpy.sign(x))
    hehe = numpy.absolute(numpy.sign(x_d1))
    jiji = numpy.absolute(numpy.sign(x_d2))
    return x, x_mask_news, x_mask_sentence, x_d1, x_d1_mask_news, x_d1_mask_sentence, x_d2, x_d2_mask_news, x_d2_mask_sentence, l, final_mask


def days(emb, sequence_mask, sentence_mask, news_mask, keep_prob, is_training, options):
    # emb batch,news, sentence,sequence,embedding, 32*4*40*20*13*100
    # sequence_mask batch, news,sentence,sequence 32*4*40*20*13
    # sentence_mask batch, news , sentence 32*4*40*20
    # news_mask batch, news, 32*4*40
    batch = tf.shape(emb)[0]
    day = tf.shape(emb)[1]
    new_s = tf.shape(emb)[2]
    sentence = tf.shape(emb)[3]
    word = tf.shape(emb)[4]
    ##TODO prepare word level input
    word_level_inputs = tf.reshape(emb,
                                   [batch * day * new_s * sentence, word, options['dim_word']])  # (32*4*40*20)*13*100
    word_level_mask = tf.reshape(sequence_mask, [batch * day * new_s * sentence, word])  # (32*4*40*20)*13
    ...
    word_encoder_out = bilstm_filter(word_level_inputs, word_level_mask, keep_prob,
                                     ...
                                     is_training=is_training)  # output shape: batch*news*sentence,sequence,2*lstm_units(32*4*40*20)*13*600
    word_encoder_out = tf.concat(word_encoder_out, 2) * tf.expand_dims(word_level_mask, -1)  # mask the output
    ##TODO word level attention
    word_level_output = attention_v1(word_encoder_out, word_level_mask, name='word_attention', keep=keep_prob,
                                     is_training=is_training)
    # word_level_output shape is (32*4*40*20)*600
    ##TODO average word
    # word_level_output = tf.reduce_sum(word_level_inputs * tf.expand_dims(word_level_mask, -1), 1) / tf.expand_dims(
    #    tf.reduce_sum(word_level_mask, 1) + 1e-8, 1)  # word_level_output shape is (32*4*40*20)*100
    if options['use_dropout']:
        word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training)
    ##TODO prepare sentence level input
    sentence_level_input = tf.reshape(word_level_output,
                                      [batch * day * new_s, sentence, 2 * options['dim']])  # (32*3*40)*20*600
    sentence_level_mask = tf.reshape(sentence_mask, [batch * day * new_s, sentence])  # (32*3*40)*20
    ##TODO setence level LSTM
    
    sentence_encoder_out = bilstm_filter(sentence_level_input, sentence_level_mask, keep_prob,
                                         prefix='sentence_encode', dim=options['dim'],
                                         is_training=is_training)  # output shape: batch*news,sentence,2*lstm_units(32*3*40)*20*600
    sentence_encoder_out = tf.concat(sentence_encoder_out, 2) * tf.expand_dims(sentence_level_mask,
                                                                               -1)  # mask the output
    ##TODO sentence level attention
    sentence_level_output = attention_v1(sentence_encoder_out, sentence_level_mask, name='sentence_attention',
                                         keep=keep_prob,
                                         is_training=is_training)
    # sentence_level_output shape is (32*3*40)*600
    '''
    ##TODO average sentence
    sentence_level_output = tf.reduce_sum(sentence_level_input * tf.expand_dims(sentence_level_mask, -1), 1) / tf.expand_dims(
        tf.reduce_sum(sentence_level_mask, 1) + 1e-8, 1)  # word_level_output shape is (32*3*40)*600
    '''
    if options['use_dropout']:
        sentence_level_output = layers.dropout(sentence_level_output, keep_prob=keep_prob, is_training=is_training)
    ##TODO prepare news level input
    news_level_input = tf.reshape(sentence_level_output, [batch * day, new_s, 2 * options['dim']])  # (32*3)*40*600
    news_level_mask = tf.reshape(news_mask, [batch * day, new_s])  # (32*3)*40
    news_level_input = news_level_input * tf.expand_dims(news_level_mask, -1)  # mask before attention
    ##TODO news level attention
    news_level_output = attention_v1(news_level_input, news_level_mask, name='news_attention', keep=keep_prob,
...
def news(emb, sequence_mask, sentence_mask, news_mask, keep_prob, is_training, options):
    # emb batch,news, sentence,sequence,embedding, 32*40*20*13*100
    # sequence_mask batch, news,sentence,sequence 32*40*20*13
    # sentence_mask batch, news , sentence 32*40*20
    # news_mask batch, news, 32*40
    batch = tf.shape(emb)[0]
    new_s = tf.shape(emb)[1]
    sentence = tf.shape(emb)[2]
    word = tf.shape(emb)[3]
    ##TODO prepare word level input
    word_level_inputs = tf.reshape(emb, [batch * new_s * sentence, word, options['dim_word']])  # (32*40*20)*13*100
    word_level_mask = tf.reshape(sequence_mask, [batch * new_s * sentence, word])  # (32*40*20)*13
    ##TODO word level LSTM

    word_encoder_out = bilstm_filter(word_level_inputs, word_level_mask, keep_prob,
                                     prefix='sequence_encode', dim=options['dim'],
                                     is_training=is_training)  # output shape: batch*news*sentence,sequence,2*lstm_units(32*40*20)*13*600
    word_encoder_out = tf.concat(word_encoder_out, 2) * tf.expand_dims(word_level_mask, -1)  # mask the output
    ##TODO word level attention
    word_level_output = attention_v1(word_encoder_out, word_level_mask, name='word_attention', keep=keep_prob,
                                     is_training=is_training)
    # word_level_output shape is (32*40*20)*600
    ##TODO average word
    # word_level_output = tf.reduce_sum(word_level_inputs * tf.expand_dims(word_level_mask, -1), 1) / tf.expand_dims(
    #    tf.reduce_sum(word_level_mask, 1) + 1e-8, 1)  # word_level_output shape is (32*40*20)*100
    if options['use_dropout']:
        word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training)
    ##TODO prepare sentence level input
    sentence_level_input = tf.reshape(word_level_output, [batch * new_s, sentence, 2 * options['dim']])  # (32*40)*20*600
    sentence_level_mask = tf.reshape(sentence_mask, [batch * new_s, sentence])  # (32*40)*20
    ##TODO setence level LSTM
    
    sentence_encoder_out = bilstm_filter(sentence_level_input, sentence_level_mask, keep_prob,
                                         prefix='sentence_encode', dim=options['dim'],
                                         is_training=is_training)  # output shape: batch*news,sentence,2*lstm_units(32*40)*20*600
    sentence_encoder_out = tf.concat(sentence_encoder_out, 2) * tf.expand_dims(sentence_level_mask,
                                                                               -1)  # mask the output
    ##TODO sentence level attention
    sentence_level_output = attention_v1(sentence_encoder_out, sentence_level_mask, name='sentence_attention',
                                         keep=keep_prob,
                                         is_training=is_training)
    # sentence_level_output shape is (32*40)*600
    '''
    ##TODO average sentence
    sentence_level_output = tf.reduce_sum(sentence_level_input * tf.expand_dims(sentence_level_mask, -1), 1) / tf.expand_dims(
        tf.reduce_sum(sentence_level_mask, 1) + 1e-8, 1)  # word_level_output shape is (32*40)*600
    '''
    if options['use_dropout']:
        sentence_level_output = layers.dropout(sentence_level_output, keep_prob=keep_prob, is_training=is_training)
    ##TODO prepare news level input
    news_level_input = tf.reshape(sentence_level_output, [batch, new_s, 2 * options['dim']])  # 32*40*600
    news_level_input = news_level_input * tf.expand_dims(news_mask, -1)  # mask before attention
    ##TODO news level attention
    news_level_output = attention_v1(news_level_input, news_mask, name='news_attention', keep=keep_prob,
...
def attention_v2(input, mask, name='attention', nin=600, keep=1.0, r=10, is_training=True):
    ...
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
        ...
        over_hidden = tf.layers.dense(outputs, nin * 2, activation=tf.nn.tanh, use_bias=False,
                                kernel_initializer=layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),
...
def build_model(embedding, options):
    ...
    with tf.device('/cpu:0'):
        with tf.variable_scope('input'):
            ##TODO x
            # news,batch,sentence,word
            x = tf.placeholder(tf.int64, shape=[None, None, None, None],
                               name='x')  # 3D vector timestep, batch and sequence(before embedding)40*32*13
            x_mask_sentence = tf.placeholder(tf.float32, shape=[None, None, None], name='x_mask_sentence')
            # mask_sentence, news, batch,sentence 40*32*10
            x_mask = tf.placeholder(tf.float32, shape=[None, None], name='x_mask')  # mask news, batch
            ##TODO y
            y = tf.placeholder(tf.int64, shape=[None], name='y')
            ##TODO x_d1
            # day,news,batch,sentence,word
            x_d1 = tf.placeholder(tf.int64, shape=[None, None, None, None, None], name='x_d1')
            x_d1_mask_sentence = tf.placeholder(tf.float32, shape=[None, None, None, None], name='x_d1_mask_sentence')
            # mask_sentence, day, news, batch,sentence
            x_d1_mask = tf.placeholder(tf.float32, shape=[None, None, None], name='x_d1_mask')
            ##TODO x_d2
            x_d2 = tf.placeholder(tf.int64, shape=[None, None, None, None, None], name='x_d2')
            x_d2_mask_sentence = tf.placeholder(tf.float32, shape=[None, None, None, None], name='x_d2_mask_sentence')
            ...
            ##TODO important
            sequence_mask = tf.cast(tf.abs(tf.sign(x)), tf.float32)  # 4D
            sequence_d1_mask = tf.cast(tf.abs(tf.sign(x_d1)), tf.float32)  # 5D
            sequence_d2_mask = tf.cast(tf.abs(tf.sign(x_d2)), tf.float32)  # 5D
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('news'):
            att = news(emb, sequence_mask, x_mask_sentence, x_mask, keep_prob, is_training, options)
        ##TODO att shape 32*600 att_day1 32*3*600 att_day2 32*4*600
        '''
        with tf.name_scope('day1'):
            att_day1 = days(emb_d1, sequence_d1_mask, x_d1_mask_sentence, x_d1_mask, keep_prob, is_training, options)
        '''
        # TODO bilstm layers
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('day2'):
            att_day2 = days(emb_d2, sequence_d2_mask, x_d2_mask_sentence, x_d2_mask, keep_prob, is_training, options)
        '''
        with tf.name_scope('final'):
            ...
            if options['last_layer'] == 'LSTM':
                ...
                #att = attention_v1(tf.concat(final, 2), final_mask, name='day_attention', keep=keep_prob,
                #                is_training=is_training)  # already masked after attention
                ##TODO take day lstm average
                # att = tf.reduce_mean(tf.concat(final,2),1)
                ##TODO take the last
                # att=final[:,-1:]
            ...
            if options['use_dropout']:
                logit = layers.dropout(logit, keep_prob=keep_prob, is_training=is_training)
...
def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, pred, summary, eidx,
    ...
    for x_sent, x_d1_sent, x_d2_sent, y_sent in iterator:
        num += len(x_sent)
        data_x, data_x_mask, data_x_mask_sentence, data_x_d1, data_x_d1_mask, data_x_d1_mask_sentence, data_x_d2, data_x_d2_mask, data_x_d2_mask_sentence, data_y, final_mask = prepare_data(
            x_sent,
            x_d1_sent,
            x_d2_sent,
            y_sent,
        ...
        loss, result, preds = sess.run([cost, correct_pred, pred],
                                       feed_dict={'input/x:0': data_x, 'input/x_mask_sentence:0': data_x_mask_sentence,
                                                  'input/x_mask:0': data_x_mask,
                                                  'input/y:0': data_y,
                                                  'input/x_d1:0': data_x_d1,
                                                  'input/x_d1_mask_sentence:0': data_x_d1_mask_sentence,
                                                  'input/x_d1_mask:0': data_x_d1_mask,
                                                  'input/x_d2:0': data_x_d2,
                                                  'input/x_d2_mask_sentence:0': data_x_d2_mask_sentence,
...
def train(
        ...
        batch_size=32,
        valid_batch_size=32,
        save_model='results/S_LSTM_models/',
        saveto='S_LSTM.npz',
        dispFreq=100,
        validFreq=1000,
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/S_LSTM_models/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/S_LSTM_models/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/S_LSTM_models/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/S_LSTM_models/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        # reload history
        if reload_ and os.path.exists(saveto):
        ...
        for eidx in range(max_epochs):
            ...
            for x, x_d1, x_d2, y in train:
                ...
                data_x, data_x_mask, data_x_mask_sentence, data_x_d1, data_x_d1_mask, data_x_d1_mask_sentence, data_x_d2, data_x_d2_mask, data_x_d2_mask_sentence, data_y, final_mask = prepare_data(
                    ...
                    model_options,
                    maxlen=maxlen)
                print(data_x.shape, data_x_d1.shape, data_x_d2.shape, final_mask.shape, data_y.shape)
                ...
                _, loss,loss_no_mean,temp_acc,l2_check = sess.run([train_op, op_loss,cost,temp_accuracy,op_L2],
                                   feed_dict={'input/x:0': data_x, 'input/x_mask_sentence:0': data_x_mask_sentence,
                                              'input/x_mask:0': data_x_mask, 'input/y:0': data_y,
                                              'input/x_d1:0': data_x_d1,
                                              'input/x_d1_mask_sentence:0': data_x_d1_mask_sentence,
                                              'input/x_d1_mask:0': data_x_d1_mask,
                                              'input/x_d2:0': data_x_d2,
            ...
            logger.debug('results/S_LSTM_models/Saved loss_plot pickle')
            with open("results/S_LSTM_models/important_plot.pickle",'wb') as handle:
