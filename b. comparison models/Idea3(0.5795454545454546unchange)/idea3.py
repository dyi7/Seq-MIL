...
def sentence_embedding(emb, sequence_mask, keep_prob, is_training, options):
    ...
    word_level_inputs = word_level_inputs * tf.expand_dims(word_level_mask, -1) 
    #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@tf.concat(word_level_inputs,2)???  -->attention
    instance_level_output = cnn_sent(word_level_inputs, options, name='instances_cnn')
    
    if options['use_dropout']:
        instance_level_output = layers.dropout(instance_level_output, keep_prob=keep_prob, is_training=is_training,seed=None)
    
    # return word_level_output
    # word_level_output shape is (32*40)*600(lstm_hidden_state) #lstm_hidden_state =600
    
    instance_level_inputs = tf.reshape(instance_level_output, [batch, instance_len, 3*options['CNN_filter']])  # 2*options['dim'] = nin!!!32*40*600  
    
    return instance_level_inputs #32*40*600
...
def instance_diffv2(l):
    "The shape of input x is:(32,N,2),with values between(0,1), the shape of output (instance_pred(i)-instance_pred(j))^2 is (32,N,N)"
    N = tf.shape(l)[1]
    l_expand = tf.expand_dims(l,2)  #32,N,1,2
    lT_expand = tf.expand_dims(l,3) #32,N,2,1
    l_tile = tf.tile(l_expand,[1,1,N,1]) #32,N,N,2
    lT_tile = tf.tile(lT_expand,[1,1,1,N])  #32,N,2,N
    Z = tf.transpose(lT_tile,[0,3,1,2])  #32,N,N,2
    dist_v2 = tf.reduce_sum(tf.square(l_tile-Z),3)  #32,N,N
    # (p2-p1)^2 + (p2-p1)^2
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        """with tf.name_scope('sentence_embedding'):
            enc = sentence_embedding(emb, sequence_mask, keep_prob, is_training, options) #无x_mask"""
            ##TODO att shape 32*40*600
        with tf.name_scope('instance_cnn_encode'):
            batch = tf.shape(emb)[0]  #32
            N = tf.shape(emb)[1] #N
            seq = tf.shape(emb)[2]
            ##TODO make instances prediction through cnn encoding + MLP
            with tf.variable_scope(name_or_scope='instance_cnn_encode', reuse=tf.AUTO_REUSE):
                word_cnn_inputs = tf.reshape(emb, [batch * N, seq, options['dim_word']])
                word_cnn_mask = tf.reshape(sequence_mask, [batch * N, seq])
                ##TODO word level CNN
                final = word_cnn_inputs * tf.expand_dims(word_cnn_mask,-1)
                #final = tf.concat(word_cnn_inputs,2) * tf.expand_dims(word_cnn_mask,-1)
                
                xavier = layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32)
                conv1 = tf.layers.conv1d(final, filters=64,kernel_size=3, padding='same', strides=1,activation=tf.nn.relu, kernel_initializer=xavier, name='conv1')  #32,XX,filters
                gmp1 = tf.reduce_max(conv1, reduction_indices=1, name='gmp1')  ##返回最大值(32,filters) global max pooling
                #gmps.append(gmp1)
                conv2 = tf.layers.conv1d(final, filters=64,kernel_size=4, padding='same', strides=1, activation=tf.nn.relu, kernel_initializer=xavier, name='conv2')
                gmp2 = tf.reduce_max(conv2, reduction_indices=1, name='gmp2')  ##返回最大值(32,filters) global max pooling
                #gmps.append(gmp2)
                conv3 = tf.layers.conv1d(final, filters=64,kernel_size=5, padding='same', strides=1,activation=tf.nn.relu, kernel_initializer=xavier, name='conv3')    
                ...
                if options['use_dropout']:
                    instance_level_output = layers.dropout(concat, keep_prob=keep_prob, is_training=is_training,seed=None)
                
                enc = tf.reshape(instance_level_output, [batch, N, 3*options['CNN_filter']])  # 2*options['dim'] = nin!!!32*40*600  
                #flat = tf.reshape(concat,[-1,3*filters])
                #p_avg = tf.nn.pool(input=values,window_shape=[2],pooling_type="MAX",padding="SAME")
                logit = tf.layers.dense(concat, 300, name='fc1')
                if options['use_dropout']:
                    logit = layers.dropout(logit, keep_prob=keep_prob, is_training=is_training,seed=None)
                fc = tf.nn.relu(logit)
            
                # cnn分类器
                inst_pred = tf.layers.dense(fc, 2, activation=None, use_bias=True,kernel_initializer=layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),name='fc2', reuse=tf.AUTO_REUSE)
                
        with tf.name_scope('Group_prediction'):  
            with tf.variable_scope(name_or_scope='Group_prediction', reuse=tf.AUTO_REUSE):
                inst_input = tf.reshape(inst_pred,[batch,N,2])
                #final_mask = x_mask
                final = bilstm_filter(inst_input,x_mask,keep_prob,prefix='group_lstm', dim=options['dim'],is_training=is_training)
                final = tf.concat(final,2) * tf.expand_dims(x_mask,-1) # maske before attention
                group_enc = attention_v2(final, x_mask, name='group_attention', keep=keep_prob,r=10,is_training=is_training)
                #32,300
                if options['use_dropout']:
                    group_enc = layers.dropout(group_enc, keep_prob=keep_prob, is_training=is_training,seed=None)
                D = group_enc.get_shape().as_list()[1]
                ...
                #group_pred = tf.nn.sigmoid(tf.matmul(group_enc,theta)+b) #32,1
                
                theta = tf.get_variable('theta', [D, 2],initializer=tf.random_normal_initializer(stddev=0.1))
                b = tf.Variable(tf.constant(0.1, shape=[2]), name="bias")
                group_pred_ = tf.add(tf.matmul(group_enc,theta),b) #32,2
                group_pred = tf.nn.softmax(group_pred_,1,name='softmax')
            ...
            x_simil = Euclidean_distance(enc)  #32,N,N   有placeholder
            #l_diff = instance_diff(preds_sig) #32,N,N   有placeholder
            l_diff_ = instance_diffv2(inst_input)
            simil_cost = tf.reduce_sum(tf.multiply(x_simil,l_diff_),[1,2]) / tf.cast(N*N,tf.float32) 
            #simil_cost = tf.reduce_mean(tf.multiply(x_simil,l_diff),[1,2])  #32,
            #group_cost = tf.square(labels-group_pred) #32
            group_cost = tf.nn.softmax_cross_entropy_with_logits_v2(logits=group_pred_, labels=labels)  #32,
            cost = simil_cost + alpha_balance * group_cost  #[32,1]
...
def train(
    ...
    with tf.variable_scope('train'):
        ...
        correct_pred = tf.equal(tf.argmax(input=group_pred, axis=1), y)  # make prediction
        logger.info("Done")
