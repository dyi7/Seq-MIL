...
from data_iterator_WLSTM import TextIterator
...
def prepare_data(sequence, sequence_d1, sequence_d2, labels, options, maxlen=None, max_word=100):
    ...
    try:
        ...
        # Make an allowance for _values to be empty
        print(str(e))
        max_sequence = 0.1  #sparse/singularity/0/1
        max_sequence_d1 = 0.1
        max_sequence_d2 = 0.1
...
def days(emb, sequence_mask, news_mask, keep_prob, is_training, options):
    ...
    news_level_input = tf.reshape(word_level_output, [batch * day, new_s, options['dim_word']])  # (32*3)*40*600  
    ...
    # shape is (32*3)*600
    day_level_output = tf.reshape(news_level_output, [batch, day, options['dim_word']])  # (32*3)*600

    return day_level_output


def news(emb, sequence_mask, news_mask, keep_prob, is_training, options):
    ...
    if options['use_dropout']:
        word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training)
    news_level_input = tf.reshape(word_level_output, [batch, new_s, options['dim_word']])  # 32*40*600
...
def lstm_filter(input, mask, keep_prob, prefix='lstm', dim=300, is_training=True):
    with tf.variable_scope(name_or_scope=prefix, reuse=tf.AUTO_REUSE):
        ...
        lstm_fw_cell = rnn.LSTMCell(dim, forget_bias=1.0, initializer=tf.orthogonal_initializer(), state_is_tuple=True)
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('final'):
            ...
            if options['last_layer'] == 'LSTM':
                ...
                logit = tf.layers.dense(att_final, 300, activation=tf.nn.tanh, use_bias=True,
...
def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, pred, summary, eidx,
    ...
    for x_sent, x_d1_sent, x_d2_sent, y_sent, y_tech in iterator:
        num += len(x_sent)
...
def train(
        ...
        batch_size=32,
        valid_batch_size=32,
        save_model='results/WLSTM_models/',
        saveto='WLSTM.npz',
        dispFreq=100,
        validFreq=1000,
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/WLSTM_models/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/WLSTM_models/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/WLSTM_models/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/WLSTM_models/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/WLSTM_models/Saved loss_plot pickle')
            with open("results/WLSTM_models/important_plot.pickle",'wb') as handle:
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        #ckpt = tf.train.get_checkpoint_state("predictive_models")
        #saver.restore(ckpt.model_checkpoint_path)
        #saver.save(sess, _s(_s(_s(save_model, "epoch"), str(best_num)), "model.ckpt")) #save_model='wlstm_results/models/'
        #saver.restore(sess, _s(_s(_s(save_model, "epoch"), str(best_num)), "model.ckpt"))
        saver.restore(sess,tf.train.latest_checkpoint(_s(_s(_s(save_model, "epoch"), str(best_num)), "model.ckpt")))
        keep_prob = 1
        is_training = False
        logger.info('=' * 80)
        ...
        if os.path.exists('wlstm_results/history_predict.npz'):
            logger.info("Load and save to history_predict.npz")
            valid_history = numpy.load('results/WLSTM_models/history_predict.npz')['valid_final_result']
            test_history = numpy.load('results/WLSTM_models/history_predict.npz')['test_final_result']
            ...
            numpy.savez('results/WLSTM_models/history_predict.npz',valid_final_result=vv,test_final_result=tt,**params)
        else:
            numpy.savez('results/WLSTM_models/history_predict.npz',valid_final_result=valid_final_result,test_final_result=test_final_result,**params)
