...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('instance_prediction'):
            ...
            #print(inst_pred)"""
            ##TODO make group prediction through linear softmax
            numerator = tf.reduce_sum(tf.square(L),1)   #tf.float32
            denom = tf.reduce_sum(L,1) #tf.float32
            group_ = numerator/denom  # Do the instance_pred linear softmax  32,  
