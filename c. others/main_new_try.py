...
def prepare_data(sequence, labels, options, maxlen=None, max_word=100):
    ...
    theta = numpy.random.random((n_samples,2*options['dim'],1))
    ##TODO for label
...
def logis_derivative(y,x):
    "Calculate derivative of tf.sigmoid(theta*x) w.r.t theta: y(1-y)x"
    y_der = tf.multiply(y,1-y)
    #y_der = tf.expand_dims(y_der,-1)
    log_result = tf.multiply(y_der,x)
    return log_result


def instances_derivative(y_dist,y_der,wij):
    "input y_der:32,N,D    input wij:32,N,N     input y_dist: 32,N,N"
    b1 = 2*tf.multiply(wij,y_dist)  #32,N,N
    a1 = tf.transpose(b1,[0,2,1])
    a = tf.matmul(a1,y_der)  #32,N,D
    b = tf.matmul(b1,y_der)
    sum_result = tf.reduce_sum(a,1)-tf.reduce_sum(b,1)  #32,D  作average??
    ###############要不要除以N^2
    result = sum_result/(tf.square(tf.shape(y_der)[1]))
    
    return result  #sum_result


def group_derivative(y_hat,y_der,y):
    "input y_hat:(32,)    input y:(32,)  input y_der:(32,N,D), the shape of output should be (32,D)"
    batch = tf.shape(y_der)[0]
    N = tf.shape(y_der)[1]
    a = 2*(y_hat-y)  #32
    #b = tf.reduce_mean(y_der,1)  #(32,D)
    b = tf.reduce_sum(y_der,1)/tf.cast(N,tf.float32) #(32,D)
    g_der = tf.multiply(a,b)     #(32,D)
    g_ = tf.reduce_sum(g_der,0)/tf.cast(batch,tf.float32)
    #g_ = tf.reduce_mean(g_der,0) #(D)
    g_final = tf.tile(tf.expand_dims(g_,0),[batch,1]) 
    return g_final  #32,D
    #return g_ #D    g_der:32,D


def logis_pred(input,para,options):
    N = tf.shape(input)[1]
    inst_pred = tf.sigmoid(tf.matmul(input,para))  #32,N,D * 32,D,1 = 32,N,1  -->32,N
    L = tf.squeeze(inst_pred)  #32,N  
    L_der = logis_derivative(inst_pred,input) #32,N,1 * 32,N,D = 32,N,D  
    #L_der = logis_derivative(L,att)
    #print(inst_pred)
    ##TODO make group prediction through average instance predictions
    group_ = tf.reduce_sum(L,1) / tf.cast(N,tf.float32) 
    #group_ = tf.reduce_mean(L,1)  # Do the instance_pred average 
    group_pred = tf.cast(tf.round(group_),tf.int64)    #32
    
    return L_der, group_pred
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        with tf.variable_scope('input'):
            ...
            theta = tf.placeholder(tf.float32,shape = [None, None,1], name = 'theta') #32,D,1
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('sentenceembedding'):
            att = sentence_embedding(emb, sequence_mask, x_mask, keep_prob, is_training, options)
        ...
        with tf.name_scope('instanceprediction'):
            ...
            # or inst_pred = tf.layers.dense(att_input,m*n,activation=tf.nn.sigmoid, name="instance_pred")
            L = tf.reshape(inst_pred,[mini_batch,N])  #32,N
            theta = tf.get_variable('theta', [mini_batch, D, 1],initializer=tf.random_normal_initializer(stddev=0.1))
            #theta_or = tf.get_variable('theta', [D, 1],initializer=tf.random_normal_initializer(stddev=0.1))
            #theta = tf.tile(tf.expand_dims(theta_or,0),[mini_batch, 1, 1]) 
            inst_pred = tf.sigmoid(tf.matmul(att,theta))  #32,N,D * 32,D,1 = 32,N,1  -->32,N
            L = tf.squeeze(inst_pred)  #32,N
            ##TODO make group prediction through average instance predictions
            group_ = tf.reduce_mean(L,1)  # Do the instance_pred average 
            group_pred = tf.cast(tf.round(group_),tf.int64)    #32"""
            ##TODO make instances prediction through softmax(sigmoid) function
            L_der, group_pred = logis_pred(att,theta,options)
            x_simil = Euclidean_distance(att)  #32,N,N   有placeholder
            l_diff = instance_diff(L) #32,N,N   有placeholder
            instance_der = instances_derivative(l_diff,L_der,x_simil)  #32,D
            y = tf.reshape(y,[-1,1])
            group_der = group_derivative(group_pred,L_der,y)  #32,D
            theta_der = instance_der + alpha_balance * group_der  
            ##TODO make group prediction through Primary instance
            # group_ = tf.reduce_max(L, 1)  #The primary one is the predition   32
            ##TODO make group prediction through BP network??
            # group_ = tf.layers.dense(L,2,activation = tf.nn.softmax,name="group_pred") #32,N -->32,2
            # print(group_) print('*'*80) print(group_pred)
            #################################################################
            ################################################################
            ...
            simil_cost = tf.reduce_sum(tf.multiply(x_simil,l_diff),[1,2]) / tf.cast(N*N,tf.float32) 
    ...
    return is_training, theta, theta_der, cost, x, x_mask, y, group_pred, summary  
...
def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, group_pred, summary, eidx,
    ...
    for x_sent, y_sent in iterator:
        num += len(x_sent)
        data_x, data_x_mask, data_theta,data_y = prepare_data(x_sent,y_sent,model_options,maxlen=maxlen)
...
def train(
    ...
    is_training, theta, theta_der, cost, x, x_mask, y, group_pred, summary = build_model(embedding, model_options)
    #is_training, cost, x, x_mask, y, n_timesteps, pred, summary = build_model(embedding, model_options)
    #summary:scalar('current_cost', tf.reduce_mean(cost))和histogram('predicted_value', group_pred)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        for eidx in range(max_epochs):
            ...
            for x,y in train:
                ...
                is_training = True
                data_x, data_x_mask,data_theta,data_y = prepare_data(x,y,model_options,maxlen=maxlen)
                print(data_x.shape, data_x_mask.shape, data_theta.shape, data_y.shape)
                ...
                _, loss,loss_no_mean,temp_acc,l2_check = sess.run([train_op, op_loss,cost,temp_accuracy,op_L2],
                                   feed_dict={'input/x:0': data_x, 'input/x_mask:0': data_x_mask,'input/theta:0': data_theta, 'input/y:0': data_y,
                ...
                loss_plot['training_acc'].append(training_acc) #加的
                '''train_summary = sess.run(summary, feed_dict={'input/x:0': data_x, 'input/x_mask:0': data_x_mask,'input/theta:0': data_theta,
                                                              'input/y:0': data_y,'input/keep_prob:0':keep_prob,'input/is_training:0':is_training,'input/alpha_balance:0': alpha_balance})
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    valid_acc, valid_loss,valid_final_result = predict_pro_acc(sess, cost, prepare_data, model_options, valid, maxlen,
                                                            ...
                                                            validate=True) #validate_writer变成none
                    ...
                    logger.debug('Valid cost  {0}'.format(valid_loss))   #only once, the last
            ...
            logger.debug('Training accuracy:  {0}'.format(1.0 * training_acc/n_samples))  ##每个epoch有一个,循环所有batchs之后的值
            logger.debug('Theta value updates as:  {0}'.format(best_theta)) 
            loss_plot['training_ep'].append(training_cost/n_samples)
            loss_plot['train_ac'].append(acc) #加的
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        logger.debug('Best theta:  {0}'.format(best_theta))
