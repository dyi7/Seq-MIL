...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/mil_rep/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/mil_rep/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/mil_rep/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/mil_rep/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/mil_rep/Saved loss_plot pickle')
            with open("results/mil_rep/important_plot.pickle",'wb') as handle:
