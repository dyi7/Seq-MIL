    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('news'):
            ...
            if options['last_layer'] == 'LSTM':
                ######## uni lstm   x_mask无dim
                avg_ind = lstm_filter(avg, x_mask, keep_prob, prefix='avg_lstm', dim=100, is_training=is_training) #32,40,newdims
                ######## bi-lstm
                #avg_ind = bilstm_filter(avg, x_mask, keep_prob, prefix='avg_lstm', dim=100,is_training=is_training)  # output shape: batch,time_step,2*lstm_unit(concate) 32*7*2new_dims
                ##TODO take day lstm average
                avg_final = tf.reduce_mean(tf.concat(avg_ind,2),1)
                ##TODO take the lasts
                #avg_final= avg_ind[:,-1,:] 
                logit = tf.layers.dense(avg_final, 150, activation=tf.nn.tanh, use_bias=True,
...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/s_lstm/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/s_lstm/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/s_lstm/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/s_lstm/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/s_lstm/Saved loss_plot pickle')
            with open("results/s_lstm/important_plot.pickle",'wb') as handle:
