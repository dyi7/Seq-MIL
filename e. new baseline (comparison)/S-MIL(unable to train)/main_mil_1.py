# -*- coding: utf-8 -*-
"""
Created on Thu Dec 24 12:35:46 2020
...
def instance_diff_v2(l):
    "The shape of input x is:(32,N,-1),with values between(0,1), the shape of output (instance_pred(i)-instance_pred(j))^2 is (32,N,N)"
    N = tf.shape(l)[1]
    l_expand = tf.expand_dims(l,1)  #32,1,N
    lT_expand = tf.expand_dims(l,2) #32,N,1
    l_tile = tf.tile(l_expand,[1,N,1])
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.name_scope('instance_encoding'):
            ...
            with tf.variable_scope(name_or_scope='instance_encoding', reuse=tf.AUTO_REUSE):
                ...
                #(32*40)*100            
                avg = tf.reshape(word_level_output, [batch, N, options['dim_word']])
                ##TODO avg shape 32*40*100
        with tf.name_scope('instance_prediction'):
            ...
            #1.@@@@@@@@@@@@@@@@@@@@@@
            L_mask = tf.reshape(inst_pred,[batch,N,-1])  #32,N,1
            ...
            #1. @@@@@@@@@@@@@@@@@@@@@@@@@
            group_output = tf.reduce_sum(L_mask * tf.expand_dims(x_mask, -1), 1) / tf.expand_dims(tf.reduce_sum(x_mask, 1) + 1e-8, 1)#32,1 
            #1.@@@@@@@@@@@@@@@@@@@@@@@@@@@
            group_ff = tf.cast(tf.ceil(group_output-0.5),tf.int64)    #32,1
            group_pred = tf.squeeze(group_ff) #32，
            
            
            ##TODO new cost
            logger.info('Building f_cost...')
            x_simil = Euclidean_distance(avg)  #32,N,N   有placeholder
...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/s-mil/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/s-mil/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/s-mil/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/s-mil/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/s-mil/Saved loss_plot pickle')
            with open("results/s-mil/important_plot.pickle",'wb') as handle:
    ...
    """with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        if os.path.exists('results/s-mil/history_predict.npz'):
            logger.info("Load and save to history_predict.npz")
            valid_history = numpy.load('results/s-mil/history_predict.npz')['valid_final_result']
            test_history = numpy.load('results/s-mil/history_predict.npz')['test_final_result']
            ...
            numpy.savez('results/s-mil/history_predict.npz',valid_final_result=vv,test_final_result=tt,**params)
        else:
            numpy.savez('results/s-mil/history_predict.npz',valid_final_result=valid_final_result,test_final_result=test_final_result,**params)
