...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('news'):
            att = news(emb, sequence_mask, x_mask, keep_prob, is_training, options)  #32*nin
            if options['last_layer'] == 'LSTM':
                logit = tf.layers.dense(att, 150, activation=tf.nn.tanh, use_bias=True,
...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/han/log_result.txt')
    # Model options
    ...
    valid = TextIterator(valid_datasets[0], valid_datasets[1],
                         ...
                         cut_word=cut_word,
                         cut_news=cut_news,
                         shuffle=False, shuffle_sentence=False)      #Shuffle = false
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/han/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/han/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/han/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/han/Saved loss_plot pickle')
            with open("results/han/important_plot.pickle",'wb') as handle:
