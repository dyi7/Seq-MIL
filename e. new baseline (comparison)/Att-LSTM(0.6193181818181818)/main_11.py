...
def lstm_filter(input, mask, keep_prob, prefix='lstm', newdim=50, is_training=True):
    with tf.variable_scope(name_or_scope=prefix, reuse=tf.AUTO_REUSE):
        sequence = tf.cast(tf.reduce_sum(mask, 1), tf.int32)
        lstm_fw_cell = rnn.LSTMCell(newdim, forget_bias=0.0, initializer=tf.orthogonal_initializer(), state_is_tuple=True)
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('news'):
            ...
            if options['last_layer'] == 'LSTM':
                ######## uni lstm   x_mask无dim
                att_ind = lstm_filter(att, x_mask, keep_prob, prefix='att_lstm', newdim=100, is_training=is_training) #32,40,newdims
                ######## bi-lstm
                #att_ind = bilstm_filter(att, x_mask, keep_prob, prefix='att_lstm', dim=100,is_training=is_training)  # output shape: batch,time_step,2*lstm_unit(concate) 32*7*2new_dims
                ##TODO take day lstm average
                att_final = tf.reduce_mean(tf.concat(att_ind,2),1)
                ##TODO take the lasts
                #att_final= att_ind[:,-1,:] 
                #if options['use_dropout']:
                #    word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training,seed=None)
                logit = tf.layers.dense(att_final, 150, activation=tf.nn.tanh, use_bias=True,
...
def train(
        ...
        batch_size=32,
        valid_batch_size=32,
        save_model='results/att_lstm1/',
        saveto='ATT_LSTM1.npz',
        dispFreq=100,
        validFreq=1000,
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/att_lstm1/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/att_lstm1/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/att_lstm1/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/att_lstm1/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/att_lstm1/Saved loss_plot pickle')
            with open("results/att_lstm1/important_plot.pickle",'wb') as handle:
