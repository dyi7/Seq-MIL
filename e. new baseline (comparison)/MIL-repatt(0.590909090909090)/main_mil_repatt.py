# -*- coding: utf-8 -*-
"""
Created on Wed Dec 16 15:41:10 2020

@author: DELL
"""

#Att(instance) + lstm(单边)
# News函数无news_mask
####### update 19/12/2020   -->nin=600, lstm/bilstm dim=300
...
def attention_aggregation(input, prob, masks, name='attention_aggregation', nin=100, keep=1.0, is_training=True):
    # input is batch,time_step,hidden_state 32*40*600 masks 32*40
    # prob is the probability of each instance within the bag: batch,time_step,1
    ...
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
        ...
        attention = tf.nn.softmax(attention, 1,name='softmax') * tf.expand_dims(masks,
                                                                -1)  # 32*40*r #mask the attention here is not really neccesary,
        results = tf.reduce_sum(prob * attention, axis=1)  # 32*600
        # outputs = tf.squeeze(tf.matmul(tf.transpose(attention, [0, 2, 1]), input))  # transpose to batch,hidden,time_step
    return results
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('instance_enc'):
            ...
            with tf.variable_scope(name_or_scope='instance_enc', reuse=tf.AUTO_REUSE):
                word_level_inputs = tf.reshape(emb, [batch * N, word, options['dim_word']])
                word_level_mask = tf.reshape(sequence_mask, [batch * N, word])
                ##TODO word level LSTM
                word_encoder_out = bilstm_filter(word_level_inputs, word_level_mask, keep_prob,prefix='sequence_encode', dim=options['dim'],is_training=is_training) #batch*news,sequence,2*lstm_units(32*40)*12*600
                ...
                if options['use_dropout']:
                    word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training,seed=None)  #32*N,D
                att = tf.reshape(word_level_output, [batch, N, 2*options['dim']]) ##32*40*100
            
        with tf.name_scope('instance_prediction'):
            temp = tf.layers.dense(word_level_output, 150,activation=tf.nn.tanh,use_bias=True,kernel_initializer=layers.xavier_initializer(uniform=True,seed=None,dtype=tf.float32),name='inst_temp', reuse=tf.AUTO_REUSE)
            if options['use_dropout']:
                temp = layers.dropout(temp, keep_prob=keep_prob, is_training=is_training,seed=None)
            
            pred_sig_ = tf.layers.dense(temp, 1, activation=None, use_bias=True,kernel_initializer=layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),name='inst_pred', reuse=tf.AUTO_REUSE) 
            inst_pred = tf.nn.tanh(pred_sig_)#32*N,1 NOT 32,N,1, float32
            ##tf.sigmoid 
            L = tf.reshape(inst_pred,[batch,N,1])  #single instance的概率
            ##_____________________________________________________________
            instance_level_input = att * tf.expand_dims(x_mask, -1)  # mask before attention
            ##TODO news level attention aggregation
            bag_repre = attention_aggregation(instance_level_input,L,x_mask, name='news_attention_aggregation', keep=keep_prob,
                                                      is_training=is_training)  # shape is 32*r
            #L_input = L * tf.expand_dims(x_mask, -1)  # 32,N,1 mask before attention
            #coef = tf.concat([L_input for i in range(2*options['dim'])], 2) ##single instance的概率 复制 2*dim次
            
        with tf.name_scope('Group_prediction'):  
            #bag_repre = tf.multiply(coef, att)
            #bag_repre = tf.reduce_mean(bag_repre, axis=1) #32,2*options['dim']
            logit = tf.layers.dense(bag_repre, 300, activation=tf.nn.tanh, use_bias=True,
                                    kernel_initializer=layers.xavier_initializer(uniform=True, seed=None,
                                                                                 dtype=tf.float32),
                                    name='group_mil', reuse=tf.AUTO_REUSE)
...
def train(
        ...
        batch_size=32,
        valid_batch_size=32,
        save_model='results/mil_repatt/',
        saveto='MIL-REPATT.npz',
        dispFreq=100,
        validFreq=1000,
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/mil_repatt/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/mil_repatt/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/mil_repatt/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/mil_repatt/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/mil_repatt/Saved loss_plot pickle')
            with open("results/mil_repatt/important_plot.pickle",'wb') as handle:
