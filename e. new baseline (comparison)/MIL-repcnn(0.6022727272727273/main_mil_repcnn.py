...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('instance_enc'):
            ...
            ##TODO make instances prediction through CNN encoding and MLP
            with tf.variable_scope(name_or_scope='instance_enc', reuse=tf.AUTO_REUSE):
                ...
                ##TODO word level CNN  
                word_level_inputs = word_level_inputs * tf.expand_dims(word_level_mask, -1) #32*N,word,emb
                #news_level_output = cnn_sent(word_level_inputs, options, name='news_cnn')
                conv1 = tf.layers.conv1d(word_level_inputs, filters=options['CNN_filter'],
                ...
                conv2 = tf.layers.conv1d(word_level_inputs, filters=options['CNN_filter'],
                ...
                conv3 = tf.layers.conv1d(word_level_inputs, filters=options['CNN_filter'],
                ...
                word_level_output=tf.concat([gmp1,gmp2,gmp3],-1) #32*N,filters*3
        
                
                if options['use_dropout']:
                    word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training,seed=None)  #32*N,D
                cnn = tf.reshape(word_level_output, [batch, N, 3*options['CNN_filter']]) ##32*40*192
        ...
        with tf.name_scope('instance_prediction'):
            ...
            L_input = L * tf.expand_dims(x_mask, -1)  # 32,N,1 mask 
            #coef = tf.concat([L for i in range(2*options['dim'])], 2)
            coef = tf.concat([L_input for i in range(3*options['CNN_filter'])], 2) ##single instance的概率 复制 2*dim次
            
        with tf.name_scope('Group_prediction'):  
            bag_repre = tf.multiply(coef, cnn)
...
def train(
        ...
        batch_size=32,
        valid_batch_size=32,
        save_model='results/mil_repcnn/',
        saveto='MIL-REPCNN.npz',
        dispFreq=100,
        validFreq=1000,
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/mil_repcnn/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/mil_repcnn/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/mil_repcnn/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/mil_repcnn/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/mil_repcnn/Saved loss_plot pickle')
            with open("results/mil_repcnn/important_plot.pickle",'wb') as handle:
