...
def news(emb, sequence_mask, keep_prob, is_training, options):
    ...
    # word_level_output shape is (32*40)*nin
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('news'):
            att = news(emb, sequence_mask, keep_prob, is_training, options)  #32,40,nin
        with tf.variable_scope('kimcnn', reuse=tf.AUTO_REUSE):
            ...
            conv1 = tf.layers.conv1d(att, filters=options['CNN_filter'],kernel_size=3, padding='same', strides=int(D/2),activation=tf.nn.relu, kernel_initializer=xavier, name='conv1')  #32,XX,filters
            gmp1 = tf.reduce_max(conv1, reduction_indices=1, name='gmp1')  ##返回最大值(32,filters) global max pooling
            conv2 = tf.layers.conv1d(att, filters=options['CNN_filter'],kernel_size=4, padding='same', strides=int(D/2), activation=tf.nn.relu, kernel_initializer=xavier, name='conv2')
            gmp2 = tf.reduce_max(conv2, reduction_indices=1, name='gmp2')  ##返回最大值(32,filters) global max pooling
            conv3 = tf.layers.conv1d(att, filters=options['CNN_filter'],kernel_size=5, padding='same', strides=int(D/2),activation=tf.nn.relu, kernel_initializer=xavier, name='conv3')    
            ...
            logit = tf.layers.dense(concat, 150, name='fc1')
            if options['use_dropout']:
    ...
    return is_training, cost, x, y, n_timesteps, preds, summary
...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/att_cnn1/log_result.txt')
    # Model options
    ...
    is_training, cost, x, y, n_timesteps, pred, summary = build_model(embedding, model_options)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/att_cnn1/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/att_cnn1/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/att_cnn1/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            for x, y in train:
                ...
                _, loss,loss_no_mean,temp_acc,l2_check = sess.run([train_op, op_loss,cost,temp_accuracy,op_L2],
                                   feed_dict={'input/x:0': data_x, 'input/y:0': data_y,
            ...
            logger.debug('results/att_cnn1/Saved loss_plot pickle')
            with open("results/att_cnn1/important_plot.pickle",'wb') as handle:
