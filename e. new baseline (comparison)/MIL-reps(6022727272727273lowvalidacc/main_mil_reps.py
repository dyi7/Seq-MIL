...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('instance_enc'):
            ...
            ##TODO make instances prediction through encoding and MLP
            with tf.variable_scope(name_or_scope='instance_enc', reuse=tf.AUTO_REUSE):
                ...
                avg = tf.reshape(word_level_output, [batch, N, options['dim_word']])  # 32*40*100
        ...
        with tf.name_scope('instance_prediction'):
            ...
            coef = tf.concat([L_input for i in range(options['dim_word'])], 2) ##single instance的概率 复制 2*dim次
            
        with tf.name_scope('Group_prediction'):  
            bag_repre = tf.multiply(coef, avg)
            bag_repre = tf.reduce_mean(bag_repre, axis=1) #32,2*options['dim_word']
...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/mil_reps/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/mil_reps/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/mil_reps/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/mil_reps/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/mil_reps/Saved loss_plot pickle')
            with open("results/mil_reps/important_plot.pickle",'wb') as handle:
