...
#S_NN2
###### Use attention_v1 instead of attention_v2 去掉r=10
...
def news(emb, sequence_mask, news_mask, keep_prob, is_training, options):
    ...
    news_level_input = tf.reshape(word_level_output, [batch, new_s, 2*options['dim']])  
    # 32*40*nin
    ##TODO average news
    news_level_output = tf.reduce_sum(news_level_input * tf.expand_dims(news_mask, -1), 1) / tf.expand_dims(tf.reduce_sum(news_mask, 1) + 1e-8, 1)
    # shape is 32*nin
...
def train(
    ...
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s: %(name)s: %(levelname)s: %(message)s",
                        filename='results/s_nn2/log_result.txt')
    # Model options
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/s_nn2/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/s_nn2/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/s_nn2/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
        ...
        for eidx in range(max_epochs):
            ...
            logger.debug('results/s_nn2/Saved loss_plot pickle')
            with open("results/s_nn2/important_plot.pickle",'wb') as handle:
