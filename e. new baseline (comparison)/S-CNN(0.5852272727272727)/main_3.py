# -*- coding: utf-8 -*-
"""
Created on Wed Dec 16 15:56:44 2020
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('news'):
            ...
            avg = tf.reshape(word_level_output, [batch, new_s, options['dim_word']])  # 32*40*100
        with tf.variable_scope('kimcnn', reuse=tf.AUTO_REUSE):
            D = avg.get_shape().as_list()[-1]
            xavier = layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32)
            conv1 = tf.layers.conv1d(avg, filters=options['CNN_filter'],kernel_size=3, padding='same', strides=int(D/2),activation=tf.nn.relu, kernel_initializer=xavier, name='conv1')  #32,XX,filters
            gmp1 = tf.reduce_max(conv1, reduction_indices=1, name='gmp1')  ##返回最大值(32,filters) global max pooling
            conv2 = tf.layers.conv1d(avg, filters=options['CNN_filter'],kernel_size=4, padding='same', strides=int(D/2), activation=tf.nn.relu, kernel_initializer=xavier, name='conv2')
            gmp2 = tf.reduce_max(conv2, reduction_indices=1, name='gmp2')  ##返回最大值(32,filters) global max pooling
            conv3 = tf.layers.conv1d(avg, filters=options['CNN_filter'],kernel_size=5, padding='same', strides=int(D/2),activation=tf.nn.relu, kernel_initializer=xavier, name='conv3')    
...
def train(
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        training_writer = tf.summary.FileWriter("results/s_cnn/logs/{}/training".format(RUN_NAME), sess.graph)
        validate_writer = tf.summary.FileWriter("results/s_cnn/logs/{}/validate".format(RUN_NAME), sess.graph)
        testing_writer = tf.summary.FileWriter("results/s_cnn/logs/{}/testing".format(RUN_NAME), sess.graph)
        sess.run(init)
        history_errs = []
